# This has changed to our version-gjy-8-8-2021
# Add class LORA(Dataset)
...
class GQA(Dataset):
    def __init__(self, root, split='train', transform=None):
        with open(f'data/gqa_{split}_original.pkl', 'rb') as f:
            self.data = pickle.load(f)
...
class LORA(Dataset):
    def __init__(self, root, split='train', transform=None):
        with open(f'data/lora_{split}.pkl', 'rb') as f:
            self.data = pickle.load(f)
        ...
        self.h = h5py.File('data/lora_{}_features.hdf5'.format(split), 'r')
        self.img = self.h['data']
    ...
    def __getitem__(self, index):
        ...
        # img = self.transform(img)
        # imgfile = f'fruit_{imgfile}'
        # id = int(imgfile.rsplit('_', 1)[1][:-4])
        img = torch.from_numpy(self.img[int(imgfile)])
