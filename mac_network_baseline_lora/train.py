...
from dataset import CLEVR, LORA, collate_data, transform, GQA
from model_gqa import MACNetwork
# from model import MACNetwork - gjy
...
def train(epoch, dataset_type):
    ...
    elif dataset_type == "lora":
        dataset_object = LORA('data/lora', transform=transform)
...
def valid(epoch, dataset_type):
    if dataset_type == "CLEVR":
        ...
        dataset_object = LORA('data/lora', 'val', transform=None)
...
if __name__ == '__main__':
    ...
    for epoch in range(n_epoch):
        ...
        with open('checkpoint/checkpoint_lora{}.model'.format(str(epoch + 1).zfill(2)), 'wb') as f:
            torch.save(net_running.state_dict(), f)

        # torch.save(net_running,f'checkpoint/checkpoint_{epoch + 1}.pth')
